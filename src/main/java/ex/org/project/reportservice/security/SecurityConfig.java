...
public class SecurityConfig {
  ...
  public SecurityFilterChain actuatorSecurityFilterChain(HttpSecurity http) throws Exception {
    http
        .securityMatcher("/api/report/v1/actuator/**")  // Match actuator path explicitly
        .authorizeHttpRequests(auth -> auth
            .requestMatchers("/api/report/v1/actuator/health").permitAll()
            .requestMatchers("/api/report/v1/actuator/shutdown").authenticated()
            .anyRequest().authenticated()
        )
        .csrf(csrf -> csrf
            .ignoringRequestMatchers("/api/report/v1/actuator/shutdown")
        )
   ...
   * 
   * Handles all non-actuator requests (lower priority than actuator chain).
   * 
   * @param http HttpSecurity configuration
  ...
  public SecurityFilterChain apiSecurityFilterChain(HttpSecurity http) throws Exception {
    http
        ...
        .securityMatcher(request -> !request.getRequestURI().startsWith("/api/report/v1/actuator"))
