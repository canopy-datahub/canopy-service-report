...
public interface HubContentMetricsRepository extends JpaRepository<HubContentMetrics, Integer> {
    ...
    @Query("SELECT new ex.org.project.reportservice.model.HubContentMetrics(hcm.center, CAST(COUNT (hcm.studyPhs) as integer),SUM(hcm.totalFileSize),"  +
            ...
            "FROM HubContentMetrics hcm WHERE hcm.reportId = :reportId group by hcm.center")
